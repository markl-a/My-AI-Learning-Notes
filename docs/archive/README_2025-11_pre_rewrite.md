...
> 📍 **新讀者請從這裡開始**：[2024-2026 AI 完整領域全景圖](./2024-2026_AI完整領域全景圖.md) — 把本 repo 22 個主題放在同一張地圖上，並標示對應目錄。
...
- [LLM應用工程](#llm應用工程)
- [AI研究前沿 2024-2025](#ai研究前沿-2024-2025) ⭐ **新增！50篇論文+程式碼**
- [實作專案集錦](#實作專案集錦)
- [相關的更新Blog](#相關的更新blog)
...
### 進階主題（11-22，2026-05 新增，2024-2026 frontier 深度分析）

| 編號 | 主題 |
|---|---|
| [11](./11.AI_Hardware_Compute/README.md) | AI Hardware / Compute Stack（GPU/TPU/HBM/液冷/Neuromorphic） |
| [12](./12.AI_For_Science/README.md) | AI for Science（AlphaFold/Evo/GNoME/GraphCast） |
| [13](./13.Robotics_Embodied_AI/README.md) | Robotics & Embodied AI（VLA、humanoid、world model） |
| [14](./14.Voice_Audio_AI/README.md) | Voice / Audio AI（Whisper/ElevenLabs/Pipecat/Suno） |
| [15](./15.Privacy_Confidential_AI/README.md) | 隱私運算與機密 AI（FL/DP/TEE） |
| [16](./16.AI_Content_Authenticity/README.md) | AI 內容真實性（C2PA/SynthID） |
| [17](./17.Causal_ML/README.md) | 因果機器學習（PyWhy/DoWhy/EconML） |
| [18](./18.GNN_Graph_Learning/README.md) | GNN/圖學習/知識圖譜（GraphRAG/LightRAG） |
| [19](./19.Synthetic_Data_Engineering/README.md) | Synthetic Data 與資料工程（distilabel） |
| [20](./20.Generative_UI/README.md) | Generative UI / AI-Native Interfaces |
| [21](./21.AI_Forecasting_Economics/README.md) | AI 能力預測學/經濟學（Epoch/METR） |
| [22](./22.Self_Improving_AI/README.md) | 自動化 AI 研究 / Self-Improving |
...
| 📚 **[RESOURCES.md](./RESOURCES.md)** | 全面資源索引 | 需要工具、API、資料集等資源 |
...
3. **[Agent工作流](./QUICKSTART.md#agent工作流-10分鐘)** - 建立自主決策的AI助手
...
- 🔬 **[研究方向路徑](./LEARNING_PATHS.md#ai研究方向持續學習)** - 深入前沿論文與演算法
...
- **Model Context Protocol（MCP）**：成為 OpenAI、Anthropic 等工具呼叫的共同標準，使用 Python SDK 就能快速建立 MCP 伺服器，相關說明新增於 Agent 工具整合章節。
...
- 嘗試執行「Responses API × LangGraph Agent」的組合範例：
  ...
  3. 進一步執行 `langgraph_agent_demo.py`，觀察耐久執行與工具呼叫紀錄如何在 LangSmith 中被追蹤。

以上整理會持續更新，若有新的模型或工具，也會在對應章節補充評測與實作紀錄。

## 演算法與資料結構

這邊主要會是我演算法練習的記錄以及閱讀的心得。雖然我不是大學教授或專家等級的，所以目前難免會有些錯誤，歡迎大家指正。

**我的演算法題練習倉庫**：[LeetcodePractice](https://github.com/markl-a/LeetcodePractice)
...
1. **[《演算法圖解》](https://www.tenlong.com.tw/products/9789864342211)**（Grokking Algorithms）
   - 適合對象：初學者
   - 特色：視覺化圖解，淺顯易懂
   - 台灣購買：天瓏書局有繁中版
...
2. **[程式碼隨想錄](https://programmercarl.com/)**：詳細的中文解析（簡中）
...
這些對於理解模型如何從資料中學習並做出預測至關重要。 關鍵概念包括機率論、隨機變數、機率分佈、期望、變異數、協方差、相關性、假設檢定、信賴區間、最大似然估計和貝葉斯推理。
...
- **基礎知識**: 這包括理解神經網路的結構，例如層、權重、偏差和激活函式（sigmoid、tanh、ReLU 等）
...
- **訓練與最佳化**: 熟悉反向傳播和不同類型的損失函式，例如均方誤差 (MSE) 和交叉熵。了解各種最佳化演算法，例如梯度下降、隨機梯度下降、RMSprop 和 Adam。
...
### 1. **卷積神經網絡 (CNN) 基礎**
- **基本結構**: 學習CNN的基本結構，包括卷積層、池化層、激活函式和全連接層。
- **經典架構**: 了解經典的CNN架構如LeNet、AlexNet、VGG、GoogLeNet、ResNet等，以及它們在ImageNet等大型資料集上的應用。
...
- **物體檢測**: 了解區域提議網絡（RPN）及其在Faster R-CNN中的應用，以及其他物體檢測方法如YOLO和SSD。
    ...
    - [YOLO: Real-Time Object Detection](https://pjreddie.com/darknet/yolo/): 了解YOLO演算法的實際應用。
...
3. **情感分析系統**：使用 LSTM 或 Transformer 分析文字情感

#### 進階項目（準備進入 LLM 領域）
1. **物件檢測系統**：使用 YOLOv8 建立實時檢測應用
2. **圖像分割應用**：使用 SAM2 進行精確分割
3. **文字生成系統**：使用 GPT-2 或小型 LLM 進行微調
...
2. **使用工具收集資料並製作成資料集**：
    ...
    - [AI時代新爬蟲：網站自動轉LLM資料，firecrawl深度玩法解讀](https://www.youtube.com/watch?v=mx06IpGRJWo)
...
* 目前號稱完全開源的預訓練模型：
    ...
    * 2.[LLM 360](https://www.llm360.ai/): 開源大語言模型框架，包含訓練和資料準備程式碼、資料、指標和模型。
...
* **因果語言建模(Causal language modeling)**: 了解因果語言建模和掩碼語言建模(causal and masked language modeling)之間的區別,以及本例中使用的損失函式。更多高效率的預訓練知識可前往 [Megatron-LM](https://github.com/NVIDIA/Megatron-LM) 或[gpt-neox](https://github.com/EleutherAI/gpt-neox)了解。
...
* **高效能運算**: 這有點超出了本文的範圍，但如果您打算從頭開始建立自己的LLMs(大語言模型)（硬體、分散式工作負載等），那麼更多有關 HPC 的知識是對你而言是必要的。
...
3. **使用 API 進行請求**
    ...
    - 例如，使用 OpenAI 的 API，可以參考以下 Python 程式碼：
      ```python
...
建立向量儲存是建立檢索增強生成（Retrieval Augmented Generation，簡稱RAG）流程的第一步。文件被加載、拆分，並使用相關的片段來產生向量表示（嵌入），這些向量表示將被存儲以便在推理過程中使用。
...
* **文檔導入 Ingesting documents**: 文檔加載器是方便的包裝器，可以處理多種格式： PDF, JSON, HTML, Markdown, 等。 它們還可以直接從一些資料庫和API（GitHub, Reddit, Google Drive, 等）檢索資料。 
* **文檔拆分 Splitting documents**: 文字拆分器將文檔拆分成較小的、語義上有意義的片段。通常最好不要在n個字符後拆分文字，而是按照標題或遞迴地拆分，並附帶一些額外的元資料。
    * [LangChain - Text splitters](https://python.langchain.com/docs/modules/data_connection/document_transformers/): LangChain實現的不同文字拆分器列表。
...
* **嵌入模型 Embedding models**: 嵌入模型將文字轉換為向量表示。這允許對語言進行更深入、更細膩的理解，這對於進行語義搜索至關重要。
...
* **向量資料庫 Vector databases**: 向量資料庫（如 [Chroma](https://www.trychroma.com/), [Pinecone](https://www.pinecone.io/), [Milvus](https://milvus.io/), [FAISS](https://faiss.ai/), [Annoy](https://github.com/spotify/annoy), 等）專為儲存嵌入向量而設計。它們支援基於向量相似性有效檢索與查詢最相似的資料。
    * [The Top 5 Vector Databases](https://www.datacamp.com/blog/the-top-5-vector-databases) by Moez Ali: 最佳和最流行的向量資料庫比較。
...
相關的 deeplearning.ai短課程:
    - [Advanced Retrieval for AI with Chroma](https://www.deeplearning.ai/short-courses/advanced-retrieval-for-ai/):這門課程專注於高級資訊檢索技術，尤其是在使用大語言模型（LLM）和向量資料庫時的應用。課程涵蓋的主題包括查詢擴展、交叉編碼重排序，以及通過嵌入適配器來改進檢索結果。
...
* **查詢建構**: 儲存在傳統資料庫中的結構化資料需要特定的查詢語言，如SQL、Cypher、元資料等。我們可以直接將用戶指令翻譯成查詢，通過查詢建構來存取資料。
    * [LangChain - Query Construction查詢建構](https://blog.langchain.dev/query-construction/): 關於不同類型查詢建構的博客文章.
    * [LangChain - SQL](https://python.langchain.com/docs/use_cases/qa_structured/sql): 教程，介紹如何利用LLMs與SQL資料庫互動，包括Text-to-SQL和可選的SQL代理。
...
---
### 5. Inference optimization 推論優化

文字生成是一個成本高昂的過程，需要昂貴的硬體設備。除了量化之外，還有各種技術被提出以最大化吞吐量並降低推論成本。
...
* **Speculative decoding 投機解碼**: 使用小型模型產生草稿，然後由更大的模型審核，以加快文字生成速度。
    * [Assisted Generation](https://huggingface.co/blog/assisted-generation) by Hugging Face: HF版本的投機解碼，這是一篇有趣的博客文章，介紹了它的工作原理及其實現程式碼。
...
* **Local deployment 本地部署**: 隱私是開源LLMs相對於私有LLMs的一個重要優勢。 本地LLM伺服器 ([LM Studio](https://lmstudio.ai/), [Ollama](https://ollama.ai/), [oobabooga](https://github.com/oobabooga/text-generation-webui), [kobold.cpp](https://github.com/LostRuins/koboldcpp), 等）利用這一優勢為本地應用提供動力。     
* **Demo deployment 演示部署**:像 [Gradio](https://www.gradio.app/) 和 [Streamlit](https://docs.streamlit.io/) 這樣的框架有助於原型應用的開發和演示的分享。您也可以輕鬆地在線上部署，例如使用 [Hugging Face Spaces](https://huggingface.co/spaces)。 
    * [Streamlit - Build a basic LLM app](https://docs.streamlit.io/knowledge-base/tutorials/build-conversational-apps): 使用Streamlit建立類似ChatGPT的基礎應用的教學。
* **Server deployment 伺服器部署**: 大規模部署LLMs需要雲端 (詳見 [SkyPilot](https://skypilot.readthedocs.io/en/latest/)) 或內部部署的基礎設施，並經常利用優化的文字生成框架，如 [TGI](https://github.com/huggingface/text-generation-inference), [vLLM](https://github.com/vllm-project/vllm/tree/main)等。
...
* **Edge deployment 邊緣(中低算力)部署**: 在受限環境中，高性能框架如 [MLC LLM](https://github.com/mlc-ai/mlc-llm) 和 [mnn-llm](https://github.com/wangzhaode/mnn-llm/blob/master/README_en.md) 可以在網頁瀏覽器、Android和iOS中部署LLM。
    * [Philschmid blog](https://www.philschmid.de/) by Philipp Schmid: 關於使用Amazon SageMaker部署LLM的高品質文章集。
...
* **Prompt hacking 提示詞攻擊**: 與提示工程相關的技術略有不同，提示詞注入（使用額外指令以劫持模型的答案）、資料/提示洩漏（檢索其原始資料/提示）和越獄（製作提示詞以繞過安全特性）都算在此範圍內。
...
* **Backdoors 後門**:  攻擊向量可以針對訓練資料本身，通過污染訓練資料（例如，使用錯誤資訊）或建立後門（觸發器在推論期間秘密的改變模型行為）。
...
| **[影片生成](./3.LLM應用工程/10.多模態生成/2.影片生成/README.md)** | Stable Video Diffusion, AnimateDiff | 1-2週 | 自動短影片生成器 |
...
2. **[自動短影片生成器](./3.LLM應用工程/10.多模態生成/4.實戰項目/README.md#項目2-自動短影片生成器)**
   - 劇本→圖片→影片→配音→音樂全自動化
   - 適合營銷、教育、娛樂場景
...
> 📊 **50篇關鍵論文 | 84%開源程式碼 | 5大核心領域**

這個模塊整理了2024-2025年AI領域最重要的技術突破與論文，每篇論文都包含完整的程式碼實現和實戰指南。
