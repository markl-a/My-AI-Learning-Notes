### 8. **標準庫與常用模塊**
   ...
   - `re`（正則表達式）
   - `collections`（高效資料結構如 `Counter`, `deque`, `defaultdict`）
...
### 2. **如何使用 `os` 模塊建立和刪除目錄？**
   - **回答：** 使用 `os.mkdir()` 建立目錄，使用 `os.rmdir()` 刪除目錄。
     ```python
     import os

     os.mkdir('new_directory')  # 建立目錄
...
### 6. **如何將字串轉換為日期對象？**
   - **回答：** 使用 `datetime.strptime()` 方法可以將字串轉換為日期對象。
     ```python
     from datetime import datetime
...
### 11. **如何使用 `re` 模塊匹配字串中的模式？請舉例說明。**
   - **回答：** 使用 `re.search()` 方法可以在字串中匹配模式。
     ```python
     import re
...
### 12. **如何使用 `re` 模塊替換字串中的模式？**
   - **回答：** 使用 `re.sub()` 方法可以替換字串中的模式。
     ```python
     import re
...
### 15. **什麼是 `collections.defaultdict`？如何使用它來處理缺少鍵的情況？**
   - **回答：** `defaultdict` 是一種字典子類，當訪問不存在的鍵時會自動建立並初始化一個新條目。
     ```python
