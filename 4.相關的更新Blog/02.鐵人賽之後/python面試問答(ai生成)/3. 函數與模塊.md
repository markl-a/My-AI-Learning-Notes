...
### 1. **如何定義和呼叫一個函數？請舉例說明。**
   - **回答：** 使用 `def` 關鍵字來定義函數，使用函式名來呼叫它。例如：
     ```python
     def greet(name):
         print(f"Hello, {name}!")
...
### 2. **什麼是預設參數？如何使用？請舉例說明。**
   - **回答：** 預設參數允許在函式呼叫時不提供該參數的情況下使用預設值。例如：
     ```python
...
### 4. **匿名函式（`lambda`）是什麼？如何使用？請舉例說明。**
   - **回答：** 匿名函式是一種沒有名稱的簡單函數，使用 `lambda` 關鍵字定義。通常用於需要短小函數的場景。
     ```python
     square = lambda x: x ** 2
...
### 11. **什麼是包（Package）？如何建立 Python 包？**
   - **回答：** 包是一個包含多個模塊的目錄，必須包含一個 `__init__.py` 文件。建立包的方法：
     - 文件結構：
       ```
       mypackage/
           __init__.py
