...
### 1. **如何使用 `print()` 進行基本的調試？為什麼這種方法有時候不夠有效？**
   - **回答：** 可以在程式碼中插入 `print()` 語句來顯示變量值和程序執行流。例如：
     ```python
     def add(a, b):
         print(f"a: {a}, b: {b}")  # 調試資訊
         return a + b

     result = add(2, 3)
     print(result)
     ```
     使用 `print()` 調試的局限性在於它需要手動添加和刪除，並且在大型程式碼庫中不易管理，也無法提供交互式的調試功能。

### 2. **什麼是 `pdb`？如何使用它來進行交互式調試？**
   - **回答：** `pdb` 是 Python 的內建調試器，可以設置斷點並在命令行中逐步執行程式碼。可以通過在程式碼中插入 `import pdb; pdb.set_trace()` 來啟用。
     ```python
     def add(a, b):
...
### 3. **如何在 Python 中設置一個斷點並啟動 `pdb` 調試器？**
   - **回答：** 可以在程式碼中使用 `pdb.set_trace()` 設置斷點，然後運行程式碼時會在斷點處暫停。
     ```python
     import pdb
...
### 5. **什麼是單元測試？為什麼需要單元測試？**
   - **回答：** 單元測試是針對應用程序中最小可測試單位（如函數或類）進行測試的過程。單元測試有助於驗證程式碼的正確性、檢查邏輯錯誤、確保修改後的程式碼不會引入新錯誤，並提高程式碼的可維護性。

### 6. **如何使用 `unittest` 模塊編寫一個簡單的單元測試？**
   - **回答：** 使用 `unittest.TestCase` 類來建立測試用例，並使用 `assert` 方法來驗證測試結果。
     ```python
     import unittest
...
### 7. **如何運行 `unittest` 測試用例？**
   - **回答：** 可以使用命令行運行測試文件：
     ...
     ```
     或者直接在測試文件中添加 `unittest.main()` 呼叫，然後運行該文件。
...
### 9. **如何使用 `pytest` 編寫和運行單元測試？**
   - **回答：** 使用簡單的函式定義測試用例，不需要繼承 `TestCase`。可以直接運行 `pytest` 命令。
     ```python
     # test_sample.py
     def add(a, b):
         return a + b
...
### 11. **如何使用 `mock` 對象在單元測試中模擬外部依賴？**
   - **回答：** 使用 `unittest.mock` 模塊中的 `patch()` 來模擬外部依賴。例如模擬 API 呼叫或資料庫查詢。
     ```python
...
### 12. **什麼是覆蓋率測試？為什麼它很重要？**
   - **回答：** 覆蓋率測試是衡量測試程式碼覆蓋被測程式碼範圍的指標。高覆蓋率意味著大部分程式碼都被測試過，有助於發現潛在的錯誤，保證程式碼品質。

### 13. **如何使用 `coverage` 工具測量 Python 程式碼的測試覆蓋率？**
   - **回答：** 使用 `coverage` 工具運行測試並生成報告。
     ```bash
     coverage run -m unittest test_module.py
     coverage report  # 生成文字報告
