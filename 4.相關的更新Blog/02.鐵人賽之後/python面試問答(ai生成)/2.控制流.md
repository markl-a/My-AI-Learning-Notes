...
### 1. **Python 中的條件語句是如何工作的？舉例說明 `if`, `elif`, 和 `else` 的用法。**
   - **回答：** Python 的條件語句用來根據條件的真偽執行不同的程式碼塊。使用 `if` 檢查條件，如果條件為真，執行 `if` 下的程式碼。如果 `if` 條件不成立，可以使用 `elif` 檢查其他條件，否則使用 `else` 執行預設的程式碼。
     ```python
     x = 10
     if x > 10:
         print("x 大於 10")
...
### 2. **`for` 循環和 `while` 循環的區別是什麼？什麼時候使用哪一種？**
   - **回答：** `for` 循環通常用於遍歷可迭代對象（如列表、字典、字串等），而 `while` 循環根據一個布爾條件反覆執行程式碼，直到條件為假。當確定要迭代的次數時，使用 `for` 循環；當次數不確定但有條件約束時，使用 `while` 循環。
...
### 4. **如何使用列表推導式來建立一個包含 1 到 10 的平方數的列表？**
   - **回答：** 列表推導式是一種簡潔的建立列表的方法。以下範例生成 1 到 10 的平方數：
     ```python
     squares = [x**2 for x in range(1, 11)]
     print(squares)  # 輸出：[1, 4, 9, 16, 25, 36, 49, 64, 81, 100]
     ```

### 5. **什麼是生成器（Generator）？如何建立一個簡單的生成器？**
   - **回答：** 生成器是一種特殊的迭代器，使用 `yield` 關鍵字來返回一個值並保存其狀態。每次呼叫生成器時，它會從上次中斷的地方繼續執行。
     ```python
...
### 6. **解釋 `try`, `except`, `finally` 的用法並提供一個示例。**
   - **回答：** `try` 用來包裹可能會引發異常的程式碼，`except` 用來處理引發的異常，`finally` 用來在無論是否發生異常的情況下都執行的程式碼。
     ```python
     try:
         x = 1 / 0
...
### 7. **什麼是迭代器（Iterator）？如何手動建立一個迭代器？**
   - **回答：** 迭代器是實現了 `__iter__()` 和 `__next__()` 方法的對象，可以使用 `next()` 函數遍歷。手動建立迭代器可以通過類來實現：
     ```python
     class MyIterator:
...
### 9. **什麼是嵌套循環？如何在 Python 中使用？**
   - **回答：** 嵌套循環是指在一個循環內部再包含另一個循環。例如，以下程式碼打印一個 3x3 的矩陣：
     ```python
     for i in range(3):
         for j in range(3):
             print(i, j)
...
### 12. **如何建立一個簡單的無限循環？如何安全地退出？**
   - **回答：** 可以使用 `while True:` 建立無限循環，通常通過 `break` 關鍵字來退出。
     ```python
     while True:
         print("運行中...")
         break  # 在這裡安全地退出
...
### 14. **什麼是上下文管理器？如何用 `with` 語句來管理文件操作？**
   - **回答：** 上下文管理器用於在進入和退出某個程式碼塊時管理資源，例如自動關閉文件。
     ```python
