...
### 5. **如何在 Git 中建立和切換到新分支？**
   - **回答：** 使用 `git branch` 建立新分支，使用 `git checkout` 切換到該分支。
     ```bash
...
### 10. **如何使用 GitHub 與他人協作開發項目？**
   - **回答：** 使用 GitHub 可以通過克隆存儲庫、建立分支、提交更改、推送到遠程、發起 Pull Request，以及審查程式碼等方式進行協作。

### 11. **什麼是 Pull Request？如何使用它來提交程式碼變更？**
   - **回答：** Pull Request 是一種向開源項目提交程式碼更改的方式。通常在 GitHub 上建立分支，完成更改後推送到遠程分支，然後發起 Pull Request 請求合併程式碼變更。

### 12. **如何在 Git 中查看變更詳細資訊？**
...
### 14. **什麼是程式碼風格規範？為什麼在協作開發中遵守程式碼風格很重要？**
   - **回答：** 程式碼風格規範是一組指導編寫一致且可讀程式碼的準則，如 PEP8。遵守程式碼風格可以提高程式碼的可讀性，減少錯誤，便於團隊協作和維護。

### 15. **什麼是 PEP8？列出一些 PEP8 規範的主要指南。**
   - **回答：** PEP8 是 Python 增強提案，定義了 Python 程式碼的風格指南。主要指南包括：
     - 使用 4 個空格作為縮進
     - 每行長度不超過 79 個字符
