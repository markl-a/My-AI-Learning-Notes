### 6. **文件與輸入輸出 (I/O)**
   ...
   - CSV、JSON 文件的處理
   - 使用 `pickle` 進行資料序列化和反序列化


### 1. **如何在 Python 中打開一個文件並讀取它的內容？請舉例說明。**
   - **回答：** 可以使用內建函式 `open()` 打開文件，使用 `read()` 方法讀取文件內容。記得在操作結束後關閉文件。
     ```python
...
### 2. **什麼是 `with` 語句？為什麼在文件操作中推薦使用 `with`？**
   - **回答：** `with` 語句用於上下文管理，確保在退出程式碼塊時自動關閉文件，避免忘記關閉文件導致的資源洩漏。
     ```python
     with open('example.txt', 'r') as file:
         content = file.read()
         print(content)
     # 不需要顯式呼叫 file.close()，`with` 會自動處理
     ```

### 3. **如何向一個文件中寫入資料？請舉例說明。**
   - **回答：** 使用 `open()` 方法以寫模式（`'w'`）打開文件，然後使用 `write()` 方法寫入資料。
     ```python
...
### 8. **如何寫入資料到 CSV 文件中？請舉例說明。**
   - **回答：** 使用 `csv.writer()` 方法寫入資料到 CSV 文件。
     ```python
     import csv
...
### 11. **什麼是 `pickle` 模塊？它的用途是什麼？**
   - **回答：** `pickle` 模塊用於序列化和反序列化 Python 物件（將對象轉換為字節流或從字節流還原）。它常用於將 Python 物件保存到文件或從文件中讀取。

### 12. **如何使用 `pickle` 將 Python 物件保存到文件？請舉例說明。**
...
### 13. **如何使用 `pickle` 從文件中加載 Python 物件？請舉例說明。**
