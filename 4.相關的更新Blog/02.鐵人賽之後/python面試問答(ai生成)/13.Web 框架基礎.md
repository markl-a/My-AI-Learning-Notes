...
### 1. **什麼是 HTTP 協議？HTTP 協議的基本工作原理是什麼？**
   - **回答：** HTTP（超文字傳輸協議）是一種基於請求-響應模式的無狀態協議，用於在客戶端和伺服器之間傳輸超文字資料（如 HTML）。客戶端向伺服器發送請求（如 GET、POST 等），伺服器處理請求並返回相應的響應（如 HTML 文件、JSON 資料）。

### 2. **HTTP 請求有哪些常見方法？請列舉並簡要說明其用途。**
   - **回答：**
     - **GET**：請求從伺服器獲取資源。
     - **POST**：向伺服器提交資料，用於建立新的資源。
     - **PUT**：向伺服器提交資料，用於更新已有資源。
     - **DELETE**：請求伺服器刪除指定資源。
     - **PATCH**：部分更新資源。

### 3. **什麼是 HTTP 狀態碼？列舉幾個常見的 HTTP 狀態碼及其含義。**
   - **回答：** HTTP 狀態碼是伺服器返回給客戶端的三位數字碼，用於表示請求的狀態。
     - **200 OK**：請求成功。
...
### 4. **如何使用 Flask 建立一個基本的 Web 應用並啟動伺服器？**
   - **回答：** 使用 Flask，可以快速建立一個簡單的 Web 應用。
     ```python
...
### 5. **如何使用 Django 建立一個新的項目並運行開發伺服器？**
   - **回答：** 使用 Django 可以快速建立一個新項目並啟動開發伺服器。
     ```bash
...
### 8. **如何在 Django 中建立一個新的應用並將其添加到項目中？**
   - **回答：** 使用 `python manage.py startapp appname` 建立應用，然後在項目的 `settings.py` 文件中將應用添加到 `INSTALLED_APPS` 列表中。
     ```bash
...
### 10. **如何在 Flask 中建立一個簡單的 RESTful API？**
   - **回答：** 使用 Flask 可以簡單地建立 RESTful API。
     ```python
...
### 11. **如何在 Django 中建立一個 RESTful API？**
   - **回答：** 使用 Django REST framework 可以建立 RESTful API。
...
### 12. **如何在 Flask 中處理 POST 請求資料？**
   - **回答：** 使用 Flask 的 `request` 對象處理 POST 請求資料。
     ```python
     from flask import Flask, request
...
### 13. **在 Django 中，如何處理表單資料提交？**
   - **回答：** 在 Django 中，可以使用 `request.POST` 來處理表單資料提交。
     ```python
...
### 14. **如何在 Flask 中設置並使用模板引擎渲染 HTML？**
   - **回答：** 使用 Flask 的 `render_template` 函數來渲染 HTML 模板。
     1. 在項目中建立 `templates` 文件夾。
     2. 建立 HTML 文件，如 `index.html`。
