...
本文將從零開始實作幾個經典的機器學習演算法,幫助您理解機器學習的核心概念和實現細節。

## 目錄
1. [線性回歸 (Linear Regression)](#線性回歸)
2. [邏輯回歸 (Logistic Regression)](#邏輯回歸)
3. [K-近鄰演算法 (K-NN)](#k-近鄰演算法)
...
線性回歸是最基礎的機器學習演算法,用於預測連續值。

### 理論基礎

目標:找到最佳的直線 `y = wx + b`,使得預測值與真實值之間的誤差最小。

損失函式(MSE): `L = (1/n) Σ(y_pred - y_true)²`
...
K-NN 是一種簡單而有效的分類演算法,基於「物以類聚」的思想。
...
決策樹是一種樹狀結構的分類演算法，通過一系列規則對資料進行劃分。
...
**資訊熵**：`H(S) = -Σ p_i * log2(p_i)`

**資訊增益**：`IG(S, A) = H(S) - Σ |S_v|/|S| * H(S_v)`
