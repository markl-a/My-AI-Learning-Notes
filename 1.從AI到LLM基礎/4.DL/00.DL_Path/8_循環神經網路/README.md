...
### 2. 文字預處理（2_text_preprocessing.ipynb）
**核心概念**:
- 文字讀取和清洗
- 分詞（Tokenization）
- 詞表構建
...
### 3. 語言模型和資料集（3_language_models_and_dataset.ipynb）
**核心概念**:
- 語言模型的數學定義
- N-gram 模型
- 序列資料的批處理策略
...
#### 3. 深入探討
```
請幫我分析 RNN 和 Transformer 在處理序列資料時的差異：
1. 計算複雜度
2. 並行化能力
3. 長程依賴建模
4. 適用場景
...
#### 1. **程式碼補全和生成** (GitHub Copilot, Cursor)
```python
# 提示：實現一個 RNN 進行情感分析
...
### Q1: RNN 和 CNN 有什麼區別？
**A**:
- **CNN**: 處理空間結構資料（圖像），使用局部連接和權重共享
- **RNN**: 處理序列資料（文字、時間序列），通過隱藏狀態捕獲時間依賴

**選擇建議**:
- 圖像處理 → CNN
- 文字、時間序列 → RNN/LSTM/Transformer
- 影片處理 → CNN + RNN 或 3D CNN
...
| 方法 | 優點 | 缺點 | 適用場景 |
|------|------|------|---------|
| 獨熱編碼 | 簡單，無需預訓練 | 高維稀疏，無語義資訊 | 小詞表，教學演示 |
