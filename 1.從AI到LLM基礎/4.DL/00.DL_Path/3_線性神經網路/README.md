...
本章節介紹深度學習的基礎：線性神經網路。在介紹複雜的深度神經網路之前，我們需要掌握神經網路訓練的基礎知識，包括模型定義、資料處理、損失函式和訓練過程。
...
| 1 | [線性回歸理論](1_linear-regression.ipynb) | 📖 理論 | ⭐ | 線性回歸數學基礎、損失函式、優化演算法 |
| 4 | [Softmax 回歸理論](4_softmax-regression.ipynb) | 📖 理論 | ⭐⭐ | 分類問題、Softmax 函數、交叉熵損失 |
| 5 | [圖像分類資料集](5_image-classification-dataset.ipynb) | 📖 理論 | ⭐ | Fashion-MNIST 資料集介紹和使用 |

### 從零實現 (From Scratch)

| 編號 | 主題 | 類型 | 難度 | 說明 |
|------|------|------|------|------|
| 2 | [線性回歸從零實現](2_linear-regression-scratch.ipynb) | 💻 實作 | ⭐⭐ | 手動實現資料生成、模型、損失、優化器 |
...
4. **第 4 天**：Softmax 回歸理論和資料集 (章節 4-5)
